...
public class ConsoleUI : IConsoleUI
{
    ...
    // Compiled regex patterns for image detection
    private static readonly Regex ImageMarkerPattern = new(
        @"\[IMAGE_GENERATED\]Saved to: (.+?)\[/IMAGE_GENERATED\]",
        RegexOptions.Compiled);
    
    private static readonly Regex ImagePathPattern = new(
        @"([A-Z]:\\[^<>:""|?*\n]+\\images\\dalle_\d+_\d+\.png)",
    ...
    public void DisplayAgentResponse(string response)
    {
        ...
        var imageMatch = ImageMarkerPattern.Match(response);
        ...
        if (imageMatch.Success)
        {
            ...
            response = ImageMarkerPattern.Replace(response, "").Trim();
        }
        else
        {
            // Look for image paths in the response (e.g., C:\...\images\dalle_*.png)
            var pathMatch = ImagePathPattern.Match(response);
